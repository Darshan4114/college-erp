...
export default async function generateSummaries({
  students,
  marksheets,
  course,
}) {
  ...
  doc.output("save", { filename: "summary.pdf" });
}
  ...
  const generateTableBodyData = () => {
    ...
    for (const sem of semesters) {
      ...
      const unselectedElectives = allSubs
        ...
        .filter((sub) => {
          const selectedElectiveIds = sem.mksList[0].student.electives.map(
            (s) => s.id
          );
